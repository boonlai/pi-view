...
async function renderRaster(image, options, signal) {
  ...
  const imageBytes = await sharp(image.data).extract({ left, top, width: regionW, height: regionH }).resize(outW, outH, { fit: "fill" }).png().toBuffer();
  signal?.throwIfAborted();
  const canvas = await sharp({ create: { width: widthPx, height: heightPx, channels: 4, background: "#00000000" } }).composite([{ input: imageBytes, left: Math.floor((widthPx - outW) / 2), top: Math.floor((heightPx - outH) / 2) }]).png().toBuffer();
  ...
  signal?.throwIfAborted();
  return cropTop || cropHeight !== heightPx ? sharp(canvas).extract({ left: 0, top: cropTop, width: widthPx, height: cropHeight }).png().toBuffer() : canvas;
}
